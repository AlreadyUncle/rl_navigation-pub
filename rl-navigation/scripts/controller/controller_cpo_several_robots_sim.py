...
actor_filename = "/home/shiqing/rl_navigation_ws/src/rl-navigation/logs/2020-05-20_20-38-30_continue_with_last_cpo_training/weights/weights_actor1000.p"
arch = 'social'
...
class CPO_Controller(object):
    ...
    def execute_action(self, robot):
        if not(robot.goal_reached):
            ...
            """
            if robot.id == 1:
                motion_command.linear.x = 1.1*max(action[0],0)
                motion_command.angular.z = action[1]
            else:
                motion_command.linear.x = 0.8*max(action[0],0)
                motion_command.angular.z = action[1]
            """
            motion_command.linear.x = max(action[0],0)
            motion_command.angular.z = action[1]
...
if __name__ == '__main__':
    try:
        ...
        # passing
        # use /home/shiqing/rl_navigation_ws/src/rl-navigation/logs/2020-05-11_02-09-49_cpo_pretrained_with_poc_fixed/weights/weights_actor1000.p
        """
        position_list.append([[1.0, 1.0 ,0.0 ], [0 ,0 ,0.25*np.pi]])
        position_list.append([[9.0, 9.0 ,0.0 ], [0 ,0 ,-0.75*np.pi]])

        goal_list.append([[9.0, 9.0 ,0.0 ], [0 ,0 ,0.25*np.pi]])
        goal_list.append([[1.0, 1.0 ,0.0 ], [0 ,0 ,-0.75*np.pi]])
        """
        """
        position_list.append([[1.0, 5.0 ,0.0 ], [0 ,0 ,0*np.pi]])
        position_list.append([[9.0, 5.0 ,0.0 ], [0 ,0 ,-np.pi]])

        goal_list.append([[9.0, 5.0 ,0.0 ], [0 ,0 ,0*np.pi]])
        goal_list.append([[1.0, 5.0 ,0.0 ], [0 ,0 ,-np.pi]])
        """

        #overtaking
        # use /home/shiqing/rl_navigation_ws/src/rl-navigation/logs/2020-05-11_02-09-49_cpo_pretrained_with_poc_fixed/weights/weights_actor950.p
        """      
        position_list.append([[1.0, 1.0 ,0.0 ], [0 ,0 ,0.25*np.pi]])
        position_list.append([[3, 3 ,0.0 ], [0 ,0 ,0.25*np.pi]])

        goal_list.append([[8.5, 8.5 ,0.0 ], [0 ,0 ,0.25*np.pi]])
        goal_list.append([[6.5, 6.5 ,0.0 ], [0 ,0 ,0.25*np.pi]])
        """
        """
        position_list.append([[1.0, 5.0 ,0.0 ], [0 ,0 ,0.1*np.pi]])
        position_list.append([[2.3, 5 ,0.0 ], [0 ,0 ,0*np.pi]])

        goal_list.append([[9, 5 ,0.0 ], [0 ,0 ,0*np.pi]])
        goal_list.append([[7.7, 5 ,0.0 ], [0 ,0 ,0*np.pi]])        
        """

        # crossing
        """
        position_list.append([[1.0, 5.0 ,0.0 ], [0 ,0 ,0*np.pi]])
        position_list.append([[5, 1 ,0.0 ], [0 ,0 ,0.5*np.pi]])
        
        goal_list.append([[9.0, 5.0 ,0.0 ], [0 ,0 ,0*np.pi]])
        goal_list.append([[5.0, 9.0 ,0.0 ], [0 ,0 ,0.5*np.pi]]) 
        """

        # four agents
        position_list.append([[5.0, 1.0 ,0.0 ], [0 ,0 ,0.5*np.pi]])
        position_list.append([[5, 9 ,0.0 ], [0 ,0 ,-0.5*np.pi]])
        position_list.append([[1.0, 5.0 ,0.0 ], [0 ,0 ,0*np.pi]])
        position_list.append([[9, 5 ,0.0 ], [0 ,0 ,-np.pi]])
        
        goal_list.append([[5.0, 9.0 ,0.0 ], [0 ,0 ,0.5*np.pi]])
        goal_list.append([[5.0, 1.0 ,0.0 ], [0 ,0 ,-0.5*np.pi]])
        goal_list.append([[9.0, 5.0 ,0.0 ], [0 ,0 ,0*np.pi]])
        goal_list.append([[1.0, 5.0 ,0.0 ], [0 ,0 ,-np.pi]])
