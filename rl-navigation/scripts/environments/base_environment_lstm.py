...
class Robot(object):
    def __init__(self, args, id, n_states):
        ...
        self.use_interaction_with_the_nearest_robot_states = False
        self.use_interaction_with_other_robots_states = True
    ...
    def get_network_state(self):
        states = [len(self.all_robots)-1] # dim = 1
        laser_data_states = self.get_laser_data_states()
        states += laser_data_states # dim = 1 + 36 = 37 
        pose_data_states = self.get_pose_data_states()
        states += pose_data_states # dim = 37 + 2 = 39 
        if (self.use_orientation_state):
            orientation_state = self.get_orientation_state()
            states += orientation_state # 39 + 1 = 40
        if (self.use_velocity_states):
            velocity_states = self.get_velocity_states()
            states += velocity_states # 40 + 2 = 42
        if (self.use_collision_radius_state):
            collision_radius_state = self.get_collision_radius_state()
            states += collision_radius_state # 42 + 1 = 43
        if (self.use_interaction_with_other_robots_states): # 43 + 8*7 = 99
            tmp = []
            closest_robot = None
            closest_distance = -1
            for robot in self.all_robots:
                if robot.id != self.id:
                    tmp.append(self.get_interaction_with_another_robot_states(robot))
                    if tmp[-1][0] >= closest_distance:
                        closest_distance = tmp[-1][0]
                        closest_robot = robot
            self.set_nearest_robot(closest_robot)
            tmp.sort(key = lambda x: x[0])
            # padding 
            for _ in range(0, Config.MAX_NUM_OTHER_AGENTS_OBSERVED - len(self.all_robots) + 1):
                states += [0]*7
            for interaction_states in tmp:
    ...
    def get_interaction_with_nearest_robot_states(self):
        ...
        delta_x = nearest_robot_position.x - self.position.x
        delta_y = nearest_robot_position.y - self.position.y
    ...
    def get_interaction_with_another_robot_states(self, robot):
        interaction_states = []
        another_robot_position = robot.position
        relative_distance_state = do_linear_transform(get_distance(self.pose_data.position, another_robot_position), self.max_clip, self.inverse_distance_states)
        interaction_states += [relative_distance_state]

        delta_x = another_robot_position.x - self.position.x
        delta_y = another_robot_position.y - self.position.y
        # transform into the local coordinates of the robot (not the nearest robot)
        transformed_delta_x = np.cos(self.orientation)*delta_x + np.sin(self.orientation)*delta_y
        transformed_delta_y = np.cos(self.orientation)*delta_y - np.sin(self.orientation)*delta_x
        transformed_delta_x_state = do_linear_transform(transformed_delta_x, self.max_clip, self.inverse_distance_states)
        ...
        interaction_states += robot.get_velocity_states()
        interaction_states += robot.get_orientation_state()
        interaction_states += robot.get_collision_radius_state()
        return interaction_states
    ...
    def execute_action(self, action):
        ...
        if (self.use_social_awareness_reward):
            ...
            # crossing
            if (relative_distance_to_goal > 3) and (relative_distance_to_nearest_robot <2) and (np.abs(delta_theta) > 0.5*np.pi) and (np.abs(delta_theta) < np.pi) and (transformed_delta_y > 0) and (transformed_delta_y < 2):
                reward += self.social_awareness_reward
...
class Environment(object):
    ...
    def reset(self):
        ...
        for robot in self.robot_list:
            robot.get_all_robots(self.robot_list)
