...
class Options():
    ...
    def __init__(self):
        ...
        parser.add_argument('--jump_start', help='If the training should be jump-started from IL', type=bool, default=True)
        parser.add_argument('--model_init', help='Model used for initialization of the weights.', type=str)
        parser.add_argument('--timesteps_per_epoch', help='Number of timesteps per epoch weight update.', type=int, default=40000)
        parser.add_argument('--n_epochs', help='Number of total training epochs.', type=int, default=1500)
        parser.add_argument('--save_weights_freq', help='Frequency of saving weights.', type=int, default=50)

        parser.add_argument('--architecture', help='Architecture to use for policy model (asl or tai). asl uses 36 laser inputs and tanh activations, \
             tai uses 10 laser inputs and relu activations, social uses 36 laser inputs, tanh activations and states needed for social awareness', type=str, default='social')
        ...
        parser.add_argument('--safety_desired_threshold', type=float, default=0.8, help='Threshold to use for average discounted safety returns.')
        ...
        parser.add_argument('--map_strategy', type=str, default='random_sampling', help='Choice of the map for the navigation task, \'random-sampling\' to choose a map randomly. Otherwise use \'map-1\', \'map-2\' etc to use a specific map.')
        ...
        parser.add_argument('--use_social_awareness_reward', type=bool, default=True, help='If true, use a reward if the robot follows certain social awareness rules such as passing, crossing and overtaking.')
        parser.add_argument('--social_awareness_reward', type=float, default=0.1, help='Additional reward for following social awareness rules')
