...
#Load the file which contains all the obstacle positions
    return list(np.load(os.path.join(rospkg.RosPack().get_path('rl_navigation'),'maps', map_name+'.npy')))
...
def get_free_position(free_map, map_resolution, map_limit, map_case, n_robots):
#Sample a point in the map which is fairly away from obstacles to set the goal or initial robot position
    ...
    for i in range(n_robots):
        while(True):
            ...
            #print([free_position_0,free_position_0])
            if(map_case == 0):
                free_position_1 += 2*int(map_limit/map_resolution)
            elif(map_case == 1):
                free_position_0 += 2*int(map_limit/map_resolution)
                free_position_1 += 2*int(map_limit/map_resolution)
            elif(map_case == 2):
                free_position_0 += 2*int(map_limit/map_resolution)
            else:
            ...
            if(free_map[free_position_0][free_position_1] == 0):
                free_map[free_position_0][free_position_1]=1
                free_position_list.append([free_position_0,free_position_1])
                break
    for i in range(n_robots):
        fp0 = free_position_list[i][0]
        fp1 = free_position_list[i][1]
        free_map[fp0][fp1]=0
        fp_transformed_list.append([np.float(fp0)*map_resolution - 2*map_limit, np.float(fp1)*map_resolution - 2*map_limit])
    return fp_transformed_list
